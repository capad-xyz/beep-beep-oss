...
| 6 · **DONE** | Session B 2026-07 | ✅ Open rooms now render from the **persistent SDK event cache** via `matrix_sdk_ui::timeline::Timeline` (see `open_room_timeline` in `matrix.rs`). No new enabling was needed in matrix-sdk **0.18**: `.sqlite_store()` already wires a `SqliteEventCacheStore` (on-disk), and `SyncService`→`RoomListService::new` already calls `client.event_cache().subscribe()`. The room event cache loads its linked chunks from SQLite on init and writes updates back — so a reopened room paints from disk with no network, then updates live via the Timeline diff stream. No experimental feature flag in 0.18. | TODO (SDK-upgrade path): the event cache is not yet exposed for tuning in 0.18 (no public knobs for cache size / retention / eager cross-room warming). Revisit after an SDK bump for `experimental_auto_backpagination`, configurable chunk limits, and multi-room prefetch — do NOT bump the SDK in Session B's scope. |
